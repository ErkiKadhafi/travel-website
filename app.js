...
function animateSlide() {
  ...
  sliders.forEach((slide, index, slides) => {
    ...
    slideTl.to(revealImg, { x: "100%" });
    slideTl.fromTo(img, { scale: 2 }, { scale: 1 }, "-=1");
    slideTl.to(revealText, { x: "100%" }, "-=.7");
    //slide scene
...
function navToggle(e) {
  if (!e.target.classList.contains("active")) {
    ...
    gsap.to(".line-1", 0.5, { rotate: "45", y: 6, background: "black" });
    gsap.to(".line-2", 0.5, { rotate: "-45", y: -6, background: "black" });
...
function detailAnimation() {
  ...
  detailSlides.forEach((slide, index, slides) => {
    const slideTl = gsap.timeline({ defaults: { duration: 1 } });
    let nextSlide = slides.length - 1 === index ? "end" : slides[index + 1];
    const nextImg = nextSlide.querySelector("img");
    const nextTxt = nextSlide.querySelector(".fashion-text");
    slideTl.fromTo(slide, { opacity: 1 }, { opacity: 0 });
    slideTl.fromTo(nextImg, { x: "50%" }, { x: "0%" }, "-=1");
    slideTl.fromTo(nextTxt, { x: "-50%" }, { x: "0%" }, "-=1");
    //detail scene
